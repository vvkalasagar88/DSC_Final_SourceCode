...
class _HomePageState extends State<HomePage> {
  ...
  Future<Uint8List?> overlaySignatureOnImage(
    ...
    try {
      ...
      final ui.ParagraphBuilder paragraphBuilder = ui.ParagraphBuilder(
        ...
        ..addText('Date & Time: $formattedTime');
      ...
      // Bottom-right position for the text block
      final double textX = original.width - paragraph.width - 20;
      final double textY = original.height - paragraph.height - 20;

      // ==========================
      // GREEN TICK AS BACKGROUND
      // ==========================
      // Draw the tick BEFORE the text so it appears behind.
      // Use a larger, semi‑transparent tick centered behind the text block.
      final Paint tickPaint = Paint()
        ..color = const Color(0x334CAF50)   // very transparent green (20% opacity)
        ..style = PaintingStyle.stroke
        ..strokeWidth = 24                  // thicker for background presence
        ..strokeCap = StrokeCap.round
        ..strokeJoin = StrokeJoin.round;

      // Calculate the center of the text block
      final double textBlockCenterX = textX + (paragraph.width / 2);
      final double textBlockCenterY = textY + (paragraph.height / 2);

      // Size of the background tick (larger than the text block)
      const double tickSize = 120;

      // Build the tick path around the center
      final Path tickPath = Path()
        ..moveTo(textBlockCenterX - tickSize * 0.4, textBlockCenterY - tickSize * 0.1)
        ..lineTo(textBlockCenterX - tickSize * 0.1, textBlockCenterY + tickSize * 0.3)
        ..lineTo(textBlockCenterX + tickSize * 0.5, textBlockCenterY - tickSize * 0.2);

      canvas.drawPath(tickPath, tickPaint);

      // Draw the signature text on top (so the tick is truly a background)
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton.icon(
                          onPressed: () => downloadSignedImagetodownloades(context),
  ...
  Future<void> downloadSignedImagetodownloades(BuildContext dialogContext) async {
    ...
    try {
      ...
      if (dialogContext.mounted) {
        AnimatedSnackBar.material(
          '✅ Image saved successfully to Downloads',
                        ...
                        Container(
                          ...
                          child: TextButton(
                            onPressed: () {
                              ...
                              // ScaffoldMessenger.of(context).showSnackBar(
                              //   const SnackBar(
                              //     content: Text('Document content cleared'),
                              //     duration: Duration(seconds: 2),
                              //   ),
                              // );
                            },
