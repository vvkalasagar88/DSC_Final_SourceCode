...
class _HomePageState
  ...
  /*
   * GENERATE IMAGE HASH HEX
   */
  Future<String?> generateHashHex()
  async {
...
''';
  ...
  void startUsbMonitoring() {
    ...
    Future.doWhile(() async {
      ...
      /*
     * STOP MONITORING
     * AFTER TOKEN CONNECTED
     */
      if (tokenConnected) {

        await addLog(
          'Token Connected - Monitoring Stopped',
        );
  ...
  Future<void> runTest(
    ...
    try {
      ...
      await addLog(
        'Found ${keyList.length} key pair(s)',
      );

      if (!mounted) return;

      /*
       * ASK USER TO PICK A KEY
       * OR CREATE A NEW ONE
       */
      final selection =
      await showKeySelectionDialog(
        keyList,
      );
  ...
  /*
   * FETCH KEY PAIRS FROM TOKEN
   */
  Future<List<Map<String, dynamic>>>
  fetchKeyPairs() async {

    final dynamic rawKeys =
    await _channel.invokeMethod(
      'listKeyPairs',
    );

    return (rawKeys as List? ?? [])
        .map(
   ...
   *
   * Returns:
   *   {'action': 'new'}                       -> create new keypair
   *   {'action': 'existing', 'handle', 'id',
   *    'label'}                               -> use selected key
   *   null                                    -> cancelled
   */
  ...
  showKeySelectionDialog(
    ...
    /*
     * LOCAL MUTABLE COPY SO THE LIST
     * CAN REFRESH AFTER A DELETE
     */
    final List<Map<String, dynamic>> localKeys =
    List<Map<String, dynamic>>.from(keys);
      ...
      appBar: AppBar(
        ...
        flexibleSpace: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color:  Color(0xFF7C3AED).withOpacity(0.45),
                blurRadius: 8,
                spreadRadius: 1,
            ...
            SizedBox(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: tokenConnected && !loading
                          ? showPinDialog
                          : null,
                      icon: loading
                          ? const SizedBox(
                      ...
                      label: Text(
                          loading ? 'Running...' : 'Run Test', style: TextStyle(fontWeight: FontWeight.bold)
                      ),
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      label: Text('Clear Logs',style: TextStyle(fontWeight: FontWeight.bold),),
                      style: ElevatedButton.styleFrom(
